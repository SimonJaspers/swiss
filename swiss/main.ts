...
export const playerWon = (player: Competitor, result: Result): boolean => {
  const [p1, p2] = result.pairing;
  const winner = result.score === Score.Player1Won ? p1 : result.score === Score.Player2Won ? p2 : null;

  if (!winner) return false;
  return player.id === winner.id;
}

export const pointsForPlayerForResult = (player: Competitor, result: Result) => {
...
const calculateBuchholzScore = (player: Competitor, rounds: Round[], playerScores: Map<string, number>): number => {
  const wins = rounds.flat().filter(result => playerWon(player, result));
  const playersInWonGames = wins.flatMap(result => result.pairing);

  return playersInWonGames.reduce(
    (total, playerOrOpponent) => {
...
export const createRanking = (
  ...
  const playerScores = new Map(tournament.map(c => [ c.id, calculateScore(c, rounds) ]));
  const tieBreakScores = new Map(tournament.map(c => [ c.id, calculateBuchholzScore(c, rounds, playerScores)]));
...
export const createPairings = (tournament: Tournament, rounds: Round[]): Pairing[] => {
  ...
  const playerHistoryIncludingSelf = new Map(tournament.map(p => [ p.id, new Set([p.id])]));

  for (const { pairing: [ p1, p2 ] } of rounds.flat()) {
