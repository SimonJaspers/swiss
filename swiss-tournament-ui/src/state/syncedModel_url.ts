 ...
 * @returns 
 */
export function createSyncedModel_url<TModel extends Record<string, ko.Subscribable<string | null>>>(
  model: TModel,
): [ TModel, () => void ] {
  // Write from URL
  const writeFromUrl = () => {
  ...
  const autoSync = ko.computed(() => {
    ...
    if (urlNeedsUpdate) {
      const newUrl = window.location.pathname + (params.length ? `?${params}` : ``);
