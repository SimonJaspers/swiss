...
const addPlayer = (name: string) => {
  const maxId = tournament().reduce(
    (max, player) => Math.max(max, Number(player.id)),
    0,
  );
...
const PlayerRegistrationFormVM = ({}) => {
  ...
  const register = () => {
    if (phase() !== "registration")
      throw new Error(
        "Tournament is not in registartion phase; cannot register player",
      );
